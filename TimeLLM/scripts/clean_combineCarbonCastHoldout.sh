...
while getopts "l:d:e:n:c:m:f:g:t:p:r:h:" opt; do
  case $opt in
    l) llm_layers=$OPTARG ;;
    d) d_model=$OPTARG ;;
...
if [ -z "$llm_layers" ] || [ -z "$d_model" ] || [ -z "$train_epochs" ] || [ -z "$num_params" ] || [ -z "$save_checkpoints" ] || [ -z "$llm_model" ] || [ -z "$gpu_id" ] || [ -z "$heldout" ]; then
  usage
fi
...
#for pred_len in 336; do
  ...
  #  for init_seed in $((11)); do  
      ...
      accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port seed_process.py \
        ...
        --data_path heldout_${heldout}_combined_clean_years_no_region.csv \
